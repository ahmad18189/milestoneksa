...
(function () {
		...
		return `
			<div class="ed-toolbar">
				...
				<div class="ed-filters">
					...
					<button type="button" class="ed-btn ed-btn-secondary" id="ed-settings" title="Settings">الإعدادات</button>
	...
	function bindEvents(root, state) {
		...
		const settingsBtn = qs(root, "#ed-settings");
		if (settingsBtn) {
			settingsBtn.onclick = function () {
				if (window.frappe && frappe.set_route) {
					frappe.set_route("Form", "Executive Dashboard Settings");
				} else {
					window.location.href = "/app/executive-dashboard-settings";
				}
			};
		}
