...
(function () {
		...
		if (!rows.length) {
			return `
				<section class="ed-zone" data-zone="projects">
					...
					<div class="ed-empty">لا توجد مشاريع نشطة وفق الإعدادات.</div>
				</section>`;
		}
		const body = rows
			.map((p) => {
				...
				return `
				<tr class="clickable" data-doctype="Project" data-name="${esc(p.name)}">
					<td><span class="ed-rag ${esc(p.rag || "grey")}"></span>${esc(p.project_name)}</td>
					<td>${esc(p.status || "")}</td>
					<td>${esc(formatPct(p.progress_actual))}</td>
					<td class="unavailable-cell">${esc(planned)}</td>
					<td>${esc(delay)}</td>
					<td>${esc(formatNumber(p.budget, 0))}</td>
					<td>${esc(formatNumber(p.actual_cost, 0))}</td>
					<td>${esc(formatNumber(p.sales, 0))}</td>
				</tr>`;
			})
			.join("");
		return `
			<section class="ed-zone" data-zone="projects">
				<div class="ed-zone-header">
					<h2>حالة المشاريع (${rows.length})</h2>
					${freshnessHtml(section.freshness)}
				</div>
					...
					<table class="ed-table">
						<thead>
							<tr>
								...
								<th>الإنجاز المخطط</th>
								<th>التأخير (يوم)</th>
								<th>الميزانية</th>
								<th>التكلفة الفعلية</th>
								<th>مبيعات المشروع</th>
							</tr>
						</thead>
						<tbody>${body}</tbody>
					</table>
	...
	function injectShadowCss(shadowRoot) {
		...
		link.rel = "stylesheet";
		link.href = "/assets/milestoneksa/css/executive_dashboard_v2.css";
